...
// ── TABLE NAME → WIKI-STYLE SECTION GROUPING ─────────────────────────────
//
// The bucket's drop_json "Table" field maps to the DropsLine template's
// section parameter.  The RS3 wiki organises drop tables in this canonical
// order (mirrored from pages like General Graardor):
//
//   100% drops  →  "Always"
//   Melee / Ranged / Magic / Combat drops  (boss-specific)
//   Unique drops
//   Armour / Weapons
//   Runes & ammunition
//   Herbs
//   Seeds
//   Stone spirits / Ores / Bars
//   Gems
//   Other / Tertiary
//   Rare drop table
//
// We use the raw Table string from the bucket and normalise it into one of
// these canonical groups so sections appear in a sensible, wiki-consistent order.
...
// Icons shown in the section header, keyed to canonical group names
const TABLE_ICONS = {
  "Always":            "⭐",
  "Main":              "⚔️",
  "Unique":            "💎",
...
// Left-border accent colours per group (mirrors wiki colour language)
const TABLE_COLORS = {
  "Always":            "#f5c518",
  "Main":              "#64b5f6",
  "Unique":            "#ba68c8",
...
/**
 * Normalise a raw bucket "Table" string into one of the canonical groups.
...
function normaliseTableName(raw) {
  ...
  if (s === "always" || s === "100%")                          return "Always";
  if (/unique/i.test(s))                                       return "Unique";
  if (/herb(?!s?seed)/i.test(s) && !/seed/i.test(s))          return "Herbs";
  if (/seed/i.test(s))                                         return "Seeds";
  if (/stone.?spirit|spirit.?stone/i.test(s))                  return "Stone Spirits";
  if (/rare.?drop|rdt/i.test(s))                               return "Rare Drop Table";
  if (/rune|ammo|arrow|bolt|dart|javelin|thrownaxe/i.test(s)) return "Runes & Ammunition";
  if (/gem|uncut|sapphire|emerald|ruby|diamond|dragonstone|onyx|zenyte|hydrix/i.test(s)) return "Gems";
  if (/armou?r|weapon|shield|sword|bow|staff|wand|helm|plate|chain|legs|glove|boot|cape/i.test(s)) return "Weapons & Armour";
  if (/ore|bar|coal|iron|steel|mithril|adamant|rune|necrit|phasma|drakolith|orichalcite/i.test(s)) return "Ores & Bars";
  if (/tertiary|clue|charm|effigy|shard|sigil/i.test(s))       return "Tertiary";
  // Generic catch-alls that many bosses use:
  if (/main|common|standard|drop/i.test(s))                    return "Main";
...
function extractTableNameFromDrop(drop) {
  ...
  const raw = drop.Table || drop["Drop Type"] || "";
  return normaliseTableName(raw);
}
...
async function lookupPage(title) {
  ...
  // ── Group rows into wiki-style ordered sections ──────────────────────────
  // Use a Map keyed by canonical group name to preserve insertion order,
  // then re-sort by TABLE_ORDER afterwards.
  const sectionsMap = new Map();
  for (const row of rows) {
    const key = row.tableName; // already normalised by extractTableNameFromDrop
  ...
  // Sort sections by TABLE_ORDER; unknown groups go to the end
  ...
  const variants = [{ label: "Drops", isHard: false, sections }];
...
function makeSectionBlock(sec, onRowClick) {
  ...
  // Use wiki-style icon and accent colour for the section header
...
// Close button — only wired up outside Alt1 (Bolt-only feature)
// The button itself is hidden when running inside Alt1 (see tryInitAlt1).
...
function tryInitAlt1() {
  ...
  // ── FIX #1: Hide the close button — it's Bolt-only ──────────────────────
  // When running inside Alt1 the close button has no effect, so we hide it
  // to avoid confusion. The settings gear stays visible.
  ...
  a1script.onload = function () {
    ...
    function tryA1lib() {
      ...
      if (a1lib && typeof a1lib.captureHoldFullRs === "function") {
        ...
        // ── FIX #3: Log which right-click OCR fonts are available ──────────
        // The RS3 right-click menu uses a custom bitmap font called
        // "Runescape Chat Bold 2" (rightclick.fontmeta.json in the Alt1
        // reader source).  A1lib ships this font internally — we probe for
        // it here so you can see in the debug log whether it is present and
        // can be used for OCR instead of the current heuristic selection.
        try {
          const fonts = a1lib.ocr.listFonts ? a1lib.ocr.listFonts() : [];
          dbg("OCR fonts available: " + JSON.stringify(fonts.map(f => f.name || f)));
          const rcFont = fonts.find(f =>
            /right.?click|chat.?bold|chatbold/i.test(f.name || f)
          );
          if (rcFont) {
            dbg("✅ Right-click font found: " + (rcFont.name || rcFont));
          } else {
            dbg("ℹ️  Right-click font not in listFonts() — it may be loaded " +
                "internally by A1lib's RightClickReader rather than exposed " +
                "via ocr.listFonts(). The examine menu text uses a different " +
                "font (the chat-area font) so the current selection logic is " +
                "correct for OCR-ing Examine tooltips.");
          }
        } catch (fontErr) {
          dbg("Font probe error: " + fontErr.message);
        }
        // ─────────────────────────────────────────────────────────────────
...
async function readExamineWindow() {
  ...
  try {
    ...
    for (let gi = gaps.length - 1; gi >= 0; gi--) {
      ...
      for (let lx = g.start - 1; lx >= 0; lx--) if (colBright[lx]) { hL = true; break; }
      for (let rx = g.end + 1; rx < SW; rx++) if (colBright[rx]) { hR = true; break; }
    ...
    for (let r = 0; r < SH; r++)
      for (let c = 0; c < cW; c++) {
        const s = (r * SW + c) * 4, d = (r * cW + c) * 4;
        cD.data[d] = SD[s]; cD.data[d + 1] = SD[s + 1];
        cD.data[d + 2] = SD[s + 2]; cD.data[d + 3] = 255;
      }
    ...
    // ── FIX #3 (continued): Font selection for Examine text OCR ───────────
    // The Examine tooltip text in RS3 is rendered in the game's standard
    // chat/interface font — NOT the right-click menu font.  The right-click
    // font ("Runescape Chat Bold 2" / rightclick.fontmeta.json) is used only
    // for the menu items themselves (Attack, Examine, etc.).
    //
    // The examine *text* that appears after "Examine" in the tooltip is in
    // the same anti-aliased font as the chatbox.  Your existing fonts
    // (aa_10px_mono, aa_12px_mono, aa_8px, etc.) already cover this.
    //
    // We attempt to load the right-click font from the fonts/ directory in
    // case you have placed a copy there, and fall back to the best available
    // chat font.  If the right-click font is present it will help with
    // menus; for examine text the chat font remains the right choice.
    let font = null;
    try {
      const fonts = window.A1lib.ocr.listFonts
        ? window.A1lib.ocr.listFonts()
        : [];
      // Prefer right-click font if available (good for menu text)
      const rcFont = fonts.find(f =>
        /right.?click|chat.?bold|chatbold/i.test(f.name || f)
      );
      // Prefer RS3 chat/interface fonts for examine tooltip text
      const chatFont = fonts.find(f =>
        /rs3|plain|small|menu|context|aa_/i.test(f.name || f)
      );
      font = rcFont || chatFont || fonts[0] || null;
      dbg("OCR using font: " + (font ? (font.name || font) : "none"));
    } catch (e) {
